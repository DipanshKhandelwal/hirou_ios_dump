...
class TaskCollectionPointTable: UIView, UITableViewDelegate, UITableViewDataSource, CLLocationManagerDelegate {
    ...
    private func setupTableView() {
        ...
        tableView.register(UINib(nibName: "TaskCollectionPointCell", bundle: nil), forCellReuseIdentifier: "taskCollectionPointCell")
...
//            self.tableView.reloadRows(at: changedIndexes, with: .right)
    ...
    func tableView(_ tableView: UITableView, willDisplay cell: UITableViewCell, forRowAt indexPath: IndexPath) {
        ...
        if(list.count > indexPath.row) {
            ...
            tcpCell.btnReport.addTarget(self, action: #selector(report(sender:)), for: .touchDown)
...
//                        self.tableView.reloadRows(at: [ IndexPath(row: sender.tag, section: 0) ], with: .automatic)
    ...
    func toggleAllTasks(sender: UIButton) {
        ...
        if( isAllCompleted(taskCollectionPoint: taskCollectionPoint)) {
            ...
            UIViewController.top()?.present(confirmAlert, animated: true, completion: nil)
        }
        else {
            self.changeAllApiCall(sender: sender)
        }
