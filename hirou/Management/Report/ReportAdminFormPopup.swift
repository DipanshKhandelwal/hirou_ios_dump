...
class ReportAdminFormPopup: UIViewController, UIPickerViewDelegate, UIPickerViewDataSource, ImagePickerDelegate {
    ...
    @IBAction func cancleReport(_ sender: Any) {
    ...
    private func showPopupSuccess() {
        let presentVC = presentingViewController
        self.dismiss(animated: false, completion: {
            let viewController = ReportSuccessViewController()
            let popup = PopupViewController(contentController: viewController, position: .center(nil), popupWidth: 430, popupHeight: 320)
            presentVC?.present(popup, animated: true, completion: nil)
        })
    }
